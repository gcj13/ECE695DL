...
class ComputationalGraphPrimerPlus(ComputationalGraphPrimer):
    def run_training_loop_multi_neuron_model(self, training_data, momentum):
        self.gamma = momentum
        self.prev_grad = {param: 0 for param in self.learnable_params}
        self.prev_bias = [0 for _ in range(self.num_layers-1)]
...
#                                               deriv_sigmoid_avg[i] for i in range(len(vars_in_layer))])
            ...
            for j,var in enumerate(vars_in_layer):
                ...
                for i,param in enumerate(layer_params):
                    ...
                    grad = gradient_of_loss_for_param * deriv_sigmoid_avg[j]
                    self.prev_grad[param] = self.learning_rate * grad + self.gamma * self.prev_grad[param]
                    self.vals_for_learnable_params[param] += self.prev_grad[param]
            self.prev_bias[back_layer_index-1] = self.learning_rate * sum(pred_err_backproped_at_layers[back_layer_index]) * sum(deriv_sigmoid_avg)/len(deriv_sigmoid_avg) + self.gamma * self.prev_bias[back_layer_index-1]
            self.bias[back_layer_index-1] += self.prev_bias[back_layer_index-1]
...
loss = cgp.run_training_loop_multi_neuron_model( training_data, 0.0 )
loss_plus = cgp.run_training_loop_multi_neuron_model(training_data, 0.99)
